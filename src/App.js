...
function App() {
  ...
  useEffect(() => {
    const fetchInventory = async (userId, setInventory) => {
      try {
        ...
        if (docSnap.exists()) {
          ...
          console.warn("No inventory data found for this user.");
        }
      } catch (error) {
    ...
    const unsubscribeAuth = onAuthStateChanged(auth, async (firebaseUser) => {
      try {
        if (firebaseUser) {
          ...
          // Fetch inventory after login
          fetchInventory(firebaseUser.uid, setInventory);
